...
void rysuj2(int szer, int wys, int** plansza,int tryb,int zebrano, int koniec_poziomu);
...
void plansza_3_kwadrat(int szer, int wys, int** plansza);
void plansza_4_U(int szer, int wys, int** plansza);
void snake_napis();
...
void petla_gry(int koniec_poziomu,int wysokosc,int szerokosc,int ksztalt_planszy,int predkosc,bool& koniec_gry,int tryb)
{
	...
	else if (ksztalt_planszy==3) plansza_3_kwadrat(szerokosc, wysokosc, plansza);
	else if (ksztalt_planszy==4) plansza_4_U(szerokosc, wysokosc, plansza);
	...
	while (koniec_gry == false)
	{
		...
		rysuj2(szerokosc, wysokosc, plansza,tryb,zebrano, koniec_poziomu);
		Sleep(predkosc);
		if (kierunek != 0) kroki++;
		...
		if (koniec_gry)
		{
			...
			if (zebrano == koniec_poziomu) cout << "\n\n  Gratulacje! \n\n  Nacisnij ENTER, aby przejsc do nastepnego poziomu ";
	...
	while (true)
	{
		...
		else if (tryb_gry == 2 and koniec_programu == false)
		{
			...
			if (koniec_gry == false)
			{
				...
				Sleep(3000);
				petla_gry(5, 40, 40, 1, 65, koniec_gry, tryb_gry);
			}
			if (koniec_gry == false)
			{
				clearScreen();
				...
				Sleep(3000);
				pkt_bonus++;
				petla_gry(5, 40, 40, 3, 65, koniec_gry, tryb_gry);
			}
			if (koniec_gry == false)
			{
				clearScreen();
				...
				Sleep(3000);
				pkt_bonus++;
				petla_gry(5, 40, 40, 4, 65, koniec_gry, tryb_gry);
			}
			if (koniec_gry == false)
			{
				clearScreen();
				...
				Sleep(3000);
				pkt_bonus++;
				petla_gry(5, 40, 40, 2, 65, koniec_gry, tryb_gry);
			}
			if (koniec_gry == false)
			{
				clearScreen();
				...
				Sleep(3000);
				pkt_bonus++;
				petla_gry(5, 30, 30, 1, 55, koniec_gry, tryb_gry);
			}
			if (koniec_gry == false)
			{
				clearScreen();
				...
				Sleep(3000);
				pkt_bonus++;
				petla_gry(5, 30, 30, 4, 55, koniec_gry, tryb_gry);
			}
			if (koniec_gry == false)
			{
				clearScreen();
				...
				Sleep(3000);
				pkt_bonus++;
				petla_gry(5, 30, 30, 2, 55, koniec_gry, tryb_gry);
			}
			if (koniec_gry == false)
			{
				clearScreen();
				...
				Sleep(3000);
				pkt_bonus++;
				petla_gry(5, 40, 40, 1, 40, koniec_gry, tryb_gry);
			}
			if (koniec_gry == false)
			{
				clearScreen();
				...
				Sleep(3000);
				pkt_bonus++;
				petla_gry(5, 40, 40, 4, 40, koniec_gry, tryb_gry);
			}
			if (koniec_gry == false)
			{
				clearScreen();
				...
				Sleep(3000);
				pkt_bonus++;
				petla_gry(5, 40, 40, 2, 40, koniec_gry, tryb_gry);
			}
		}
...
void menu_glowne(int& szerokosc,int& wysokosc,int& predkosc,int&ksztalt_planszy,bool &koniec_gry,bool& koniec_programu,int& tryb)
{
	...
	snake_napis();
	pkt_wyswietl();
	cout <<endl<< "  Wpisz odpowiednia cyfre i nacisnij ENTER" << endl << endl;
	cout << "  1. Tryb normalny" << endl << endl;
	cout << "  2. Zmien poziom trudnosci" << endl << endl;
	cout << "  3. Zmien rodzaj planszy" << endl << endl;
	cout << "  4. Zmien rozmiar planszy" << endl << endl;
	cout << "  5. Przygoda" << endl << endl << endl;
	cout << "  8. Informacje" << endl << endl;
...
void wybierz_predkosc(int& predkosc)
{
	...
	do
	{
		...
		cout << "  (1)   bardzo wolno        " << endl;
		cout << "  (2)   wolno               " << endl;
		cout << "  (3)   normalnie (domyslnie)" << endl;
		cout << "  (4)   szybko              " << endl;
		cout << "  (5)   bardzo szybko       " << endl << endl;
	...
	if (wybor == 1) predkosc = 100;
	else if (wybor == 2) predkosc = 70;
	else if (wybor == 3) predkosc = 55;
	else if (wybor == 4) predkosc = 40;
	else if (wybor == 5) predkosc = 25;
}
...
int wybierz_plansze()
{
	...
	do
	{
		...
		cout << "  *       *" << endl;
		cout << "  *********" << endl << endl;
		cout << "  Trzecia pierwsza(3)" << endl << endl;
		cout << "  *********" << endl;
		cout << "  *       *" << endl;
		cout << "  *       *" << endl;
		cout << "  *  ***  *" << endl;
		...
		cout << "  *       *" << endl;
		cout << "  *********" << endl << endl;
		cout << "  Czwarta pierwsza(4)" << endl << endl;
		cout << "  *********" << endl;
		cout << "  *       *" << endl;
		cout << "  *       *" << endl;
		cout << "  *  ***  *" << endl;
...
void wypisz_informacje()
{
	...
	do
	{
		...
		cout << "  Klawisz E pozwala przerwac gre" << endl << endl;
...
void wczytaj_wynik()
{
	...
	{
		plik >> pkt_maks;
		plik >> pkt_przygoda_maks;
	}
	plik.close();
}
void zapisz_wynik()
{
	ofstream plik("punkty.txt");
	if (plik.is_open() == true)
	{
		plik << pkt_maks << endl;
		plik << pkt_przygoda_maks;
...
void rysuj2(int szer, int wys, int** plansza,int tryb,int zebrano, int koniec_poziomu) //wykorzystanie string duzo przyspiesza wypisywnie
{
	string linia;
	linia += "\n \n";
	for (int i = 0;i < wys;i++)
	...
	{
		linia += "\n  Zebrano: " + to_string(zebrano) + "$ ("+to_string(koniec_poziomu)+")";
...
void plansza_3_kwadrat(int szer, int wys, int** plansza)
{
	for (int i = 0; i < wys; i++)
	{
		for (int j = 0; j < szer; j++)
		{
			...
			if (i >= (wys / 2 - (wys / 6)) and i <= (wys / 2 + (wys / 6)))
				if (j >= (wys / 2 - wys / 6) and j <= (wys / 2 + wys / 6))
					plansza[i][j] = 1;
		}
	}
}
	...
	for (int i = 0; i < wys; i++)
	{
		for (int j = 0; j < szer; j++)
		{
			...
			if (i == (wys / 2 - (wys / 4))) 
				if (j >= (wys / 2 - wys / 6) and j <= (wys / 2 + wys / 6))
					plansza[i][j] = 1;
			if (i >= (wys / 2 - wys / 4) and i <= (wys / 2 + wys / 6))
				if (j == (wys / 2 - wys / 6) or j == (wys / 2 + wys / 6))
					plansza[i][j] = 1;
